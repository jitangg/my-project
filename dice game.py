...
def tail_points(opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    score = 2*abs((opponent_score//10%10)-(opponent_score%10))+1
    return score
    # END PROBLEM 2
...
def perfect_square(score):
    check_num = 1
    while score >= check_num:
        if score == check_num**2:
            return True
        check_num += 1
    return False
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score1 <= 100:
        score2 = 0
        while score2 <= 100:
            if strategy(score1,score2) != strategy(0,0):
                return False
            score2 += 1
        score1 += 1
    return True
...
def make_averaged(original_function, total_samples=1000):
    ...
    def average(*args):
        i = 0
        a = 0
        while total_samples > a:
            i +=original_function(*args)
            a += 1
        averg = i/total_samples
        return averg
    return average
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    result = square_update(0, score, opponent_score, dice=six_sided)
