...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            # ant already there with place.ant
            # new ant is self
            # if the ant already there can contain the new ant
                # BOTH ANTS OCCUPY THE PLACE
                # container ant (original) contain the ant added
                # no need to update ant attributre of place
            # if the ant added can contain the ariginal ant
                # BOTH ANTS OCCUPY THE PLACE
                # container ant (original) contain the ant added
                # update ant atribute of place
    ...
    def double(self):
        ...
        if self.doubled == False:
            self.damage *= 2
            self.doubled = True  #it turns to true bc it already been doubled
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place_thrower = self.place
        step_inside = 0
        while place_thrower:
            if place_thrower.is_hive == False:
                if len(place_thrower.bees) > 0 and self.upper_bound >= step_inside >= self.lower_bound:
                    return random_bee(place_thrower.bees)  # REPLACE THIS LINE
            place_thrower = place_thrower.entrance
            step_inside += 1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= amount:
            ...
            self.__class__.__bases__[0].reduce_health(self, amount)
        else:
            self.__class__.__bases__[0].reduce_health(self, amount)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if self.turns_to_chew > 0: #is Ant chewing? (it's chewing rn!)
            self.turns_to_chew -= 1
        elif self.turns_to_chew == 0: #ant is not chewing rn
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        ...
        #other ant isn't container(can use is_container attribute)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while tile:
            if tile.ant:
                if tile.ant.is_container and tile.ant.ant_contained:
                    tile.ant.ant_contained.double()
                tile.ant.double()
            tile = tile.exit
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def slow_action(gamestate):
            """The action of a slowed bee. It moves every other turn and takes no action otherwise."""
            if target.slow_turns:
                target.slow_turns -= 1
                if target.slow_turns:
                    if gamestate.time % 2 == 0 and target.health > 0 and target.place is not None:
                        target.move_to(target.place.exit)
                else:
                    target.is_slow = False
                    target.action = target.old_action

        target.old_action = target.action
